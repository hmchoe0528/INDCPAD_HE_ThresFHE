// From: he/hefloat/bootstrapping/bootstrapping.go
...
func (eval Evaluator) bootstrap(ctIn *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, errScale *rlwe.Scale, err error) {
	...
	// fmt.Printf("\nbootstrapping.go:: Done.. step 1: ScaleDown\n")

	// Step 2 : Extend the basis from q to Q
	if ctOut, err = eval.ModUp(ctOut); err != nil {
		return
	}
	// fmt.Printf("\nbootstrapping.go:: Done.. step 2: ModRaise\n")
	...
	// fmt.Printf("\nbootstrapping.go:: Done.. step 3: CoeffsToSlots\n")
	...
	// fmt.Printf("\nbootstrapping.go:: Done.. step 4: EvalMod\n")
	// fmt.Printf("bootstrapping.go:: after EvalMod, IsNTT: %t, IsMont: %t", ctReal.IsNTT, ctReal.IsMontgomery)

	// Step 5 : SlotsToCoeffs (Homomorphic decoding)
	if ctOut, err = eval.SlotsToCoeffs(ctReal, ctImag); err != nil {
		return
	}
	// fmt.Printf("\nbootstrapping.go:: Done.. step 5: SlotsToCoeffs\n")
...
// Custom, encapsulate by user.
// ModUp raise the modulus from q to Q, scales the message  and applies the Trace if the ciphertext is sparsely packed.
func (eval Evaluator) ModUpCustom(ctIn *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, err error) {
