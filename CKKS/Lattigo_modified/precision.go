// From: schemes/ckks/precision.go
...
func getPrecisionStatsF64(params Parameters, encoder *Encoder, decryptor *rlwe.Decryptor, want, have interface{}, logprec float64, computeDCF bool) (prec PrecisionStats) {
	...
	for i := range valuesWant {
		...
		if precReal[i] < 0 {
			...
			// fmt.Println(precReal[i], " at ", i)
		}

		if deltaReal > MaxDeltaReal {
			MaxDeltaReal = deltaReal
			// fmt.Println(MaxDeltaReal, "at i=", i, " and", real(valuesWant[i]), "/", real(valuesHave[i]))
		}
	...
	if len(indexList) < 100 {
		if len(indexList) == 0 {
			fmt.Println("precision.go:: no failure!")
		} else {
			fmt.Println("precision.go:: indexes: ", indexList)
			for i := range valuesWant {
				deltaReal = math.Abs(real(valuesHave[i]) - real(valuesWant[i]))
				precReal[i] = -math.Log2(deltaReal)
				deltaImag = math.Abs(imag(valuesHave[i]) - imag(valuesWant[i]))
				precImag[i] = -math.Log2(deltaImag)
				if precReal[i] < 0 {
					fmt.Println("At ", i, ":", precReal[i], "(", deltaReal, ")")
				}
				if precImag[i] < 0 {
					fmt.Println("At ", i, ":", precReal[i], "(", deltaImag, ")")
				}
			}
		}
	} else {
		fmt.Println("precision.go:: index too long: ", len(indexList))
		// for i := 1; i <= 25; i++ {
		// 	deltaReal = math.Abs(real(valuesHave[i]) - real(valuesWant[i]))
		// 	precReal[i] = -math.Log2(deltaReal)
		// 	if precReal[i] < 0 {
		// 		fmt.Println("At ", i, ":", deltaReal)
		// 	}
		// }
	}
...
func (prec *PrecisionStats) calcCDFF64(precs []float64, res []struct {
	...
	fmt.Printf("minprec and sorted: %f, %f, %f, %f,..", sortedPrecs[0], sortedPrecs[1], sortedPrecs[2], sortedPrecs[3])
